 ...
 * 固定签库，随机抽取：每次抽签由调用方传入一个随机种子，抽取结果不可预测，
...
object LotDraw {
    ...
    // ---------- 5. 圣经掣签 ----------
    private data class BibleLot(val reference: String, val text: String)
    private val BIBLE = listOf(
        BibleLot("约书亚记 1:9", "你当刚强壮胆！不要惧怕，也不要惊惶，因为你无论往哪里去，耶和华你的神必与你同在。"),
        BibleLot("罗马书 8:28", "我们晓得万事都互相效力，叫爱神的人得益处，就是按他旨意被召的人。"),
        BibleLot("马太福音 6:34", "不要为明天忧虑，因为明天自有明天的忧虑。一天的难处一天当就够了。"),
        BibleLot("诗篇 46:1", "神是我们的避难所，是我们的力量，是我们在患难中随时的帮助。"),
        BibleLot("马太福音 7:7", "你们祈求，就给你们；寻找，就寻见；叩门，就给你们开门。"),
        BibleLot("希伯来书 11:1", "信就是所望之事的实底，是未见之事的确据。"),
        BibleLot("箴言 3:5-6", "你要专心仰赖耶和华，不可倚靠自己的聪明，在你一切所行的事上都要认定他，他必指引你的路。"),
        BibleLot("哥林多前书 13:13", "如今常存的有信，有望，有爱，这三样，其中最大的是爱。"),
        BibleLot("腓立比书 4:6", "应当一无挂虑，只要凡事藉着祷告、祈求和感谢，将你们所要的告诉神。")
    )
    ...
    fun draw(system: LotSystem, seed: Int): LotResult {
        ...
        return when (system) {
            ...
            LotSystem.BibleLot -> {
                ...
                LotResult(system, date, "经文掣签", listOf(
                    "经文" to m.text,
                    "出处" to m.reference,
                    "读法" to "建议回到该出处的前后文阅读；此结果可作为安静思考的提示。"
                ))
            }
