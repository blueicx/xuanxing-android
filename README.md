...
- **综合 / 东方（五行八字）**
  ...
  - 今日行动：用五行喜用、太阳星座、当日综合盘面和季节/手动城市标签计算具体菜名、行动时段和出游环境；固定权重、离线可复算，同分只做稳定排序
- **西方（星座）**
...
- **人物交流 · 中国象棋**：在综合/东方/西方页的对话面板中与角色真实对弈中国象棋
  ...
  - 深陪伴对话默认离线：支持主题/实体分析、连续追问澄清、可撤回的本地软标签；在线 Provider 只保留显式扩展接缝，外部文本需经过本地事实与安全校验
  - 舞台标题只显示「玄学家」或「半仙」；文化皮肤通过场景、道具、动作和语汇区分，不把文化演绎宣称为真实仪式
- **我的 · 命盘设置**：出生日期 / 时间 / 地点编辑并保存（DataStore 本地存储）
  - 人生画像：职业倾向簇、色彩灵感、环境类型和最多 3 个国家/城市匹配示例；测试未完成时明确显示低置信度与缺失输入
  - 饮食偏好：素食、清真、忌辣、忌酒和自定义排除项只保存在本机并按档案隔离
...
今日行动与人生画像是文化/生活方式灵感，不构成医疗、过敏、营养治疗、心理诊断、投资、移民、旅行安全或收入建议。城市采用手动选择与离线静态标签，不请求 GPS、不接地图或天气 API；在线 Provider 仍只是显式扩展 seam，不能覆盖本地盘面事实或安全边界。
