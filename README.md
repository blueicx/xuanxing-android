...
- **人物交流 · 中国象棋**：在综合/东方/西方页的对话面板中与角色真实对弈中国象棋
  - 纯 Kotlin 规则引擎：合法走法、吃子、将军/将死/困毙、悔棋、提示、复盘，全部经过本地规则校验
  - 角色解说只引用真实棋盘事件（棋子/坐标/吃子/胜负），不结合运势下结论；「保存棋局」只存棋谱不存评语
  - 默认离线确定性应手（不显示胜率/等级）；围棋与国际象棋仅预留接口，显示「尚未启用」
  - 棋盘 9×10 可点选走子，悔棋/提示/退出按钮 ≥48dp 并带 TalkBack 描述
...
棋局边界：中国象棋规则与离线应手完全本地运行；角色话术只基于棋盘事实。Pikafish 原生引擎、围棋 GTP、国际象棋 UCI 均为预留接口，未打包二进制、不联网；接入前需完成对应开源许可（GPLv3 等）与 source offer（见 `NOTICE-THIRD-PARTY.md` 与 `docs/BOARD_GAME_INTEGRATION.md`）。
