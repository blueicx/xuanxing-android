...
persona 命名漂移（已收口，见「当前基线」第 7 条）：同两个模式曾分别在 `MysticGuideCard.kt` 叫「玄学家 / 半仙」、在 `MysticFloatingGuide.kt` 叫「慈翁 / 魔师」，`MysticOrb.kt` 的 TalkBack 里还有第三种「玄师」，且浮球动画速度靠字符串比较 `"魔师"` 分支。现在称谓只有 `MysticGuideGenerator.personaName` 一个来源，「玄师」固定为统称，动画按模式分支，退役名由 `_dev/dialogue_contract_test.js` 逐行扫描 main 源码树（`app/src/main/java/com/xuanji/app`）把关。仍开放的同源问题：`TodayOracle` 用显示名当查询键。
